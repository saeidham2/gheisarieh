...
class Product extends Component
{
    ...
    {
//        $banner = DB::connection('mysql-vehicle')->table('category_vehicle_product_swiper')->insert([
        $banner = DB::table('category_vehicle_product_swiper')->insert([
...
//        $products = $this->readyToLoad ? DB::connection('mysql-vehicle')
        $products = $this->readyToLoad ? DB::table('category_vehicle_product_swiper')
            ->where('title_id', 'LIKE', "%{$this->search}%")->
            orWhere('id', $this->search)->
            latest()->paginate(15) : [];
        $titles =  DB::table('category_vehicle_title_swiper')->get();
